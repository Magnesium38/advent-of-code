...
mod addtional_tests {
	...
	#[test]
	fn states_are_equivalent_based_on_the_hallway_and_rooms_but_not_cost() {
		let mut a = parse::<2>(&make_input("ABCDABCD"));
		let mut b = parse::<2>(&make_input("ABCDABCD"));
		assert_eq!(a, b);
		...
		a.hallway[0] = Some(Amphipod::Bronze);
		a.hallway[1] = None;
		b.hallway[0] = None;
		b.hallway[1] = Some(Amphipod::Bronze);
		assert_ne!(a, b);
	}
}
